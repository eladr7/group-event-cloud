...
class MoviesContainer
{
  ...
  @task async getMoviesList() {
    await fetch("https://server-ugdhobbyma-uw.a.run.app/movies/getMovies/" + this.genre)
      .then((data) => data.json())
      .then((res) => {
          const data = res.data;
  ...
  deleteVideoFromDb(movieToDeleteId) {
    axios.delete('https://server-ugdhobbyma-uw.a.run.app/movies/deleteMovie/' + this.genre, {
      data: {
        id: movieToDeleteId,
      },
    });
  ...
  putMovieToDb(nextId, videoName, url) {
    axios.post('https://server-ugdhobbyma-uw.a.run.app/movies/putMovie/' + this.genre, {
      id: nextId,
      name: videoName,
      image: url,
    });
